...
func TestVerifyVC_Participant(t *testing.T) {
	...
	tests := []test{
		...
		{testName: "HTTPS issuer matching configured trusted participant URL should be trusted.",
			credentialToVerifiy: getCredential("https://issuer.example.com"),
			verificationContext: TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "ebsi", Url: "https://issuer.example.com"}}}},
			expectedResult:      true},
		{testName: "HTTPS issuer not matching any configured trusted participant URL should be rejected.",
			credentialToVerifiy: getCredential("https://untrusted-issuer.example.com"),
			verificationContext: TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "ebsi", Url: "https://issuer.example.com"}}}},
			expectedResult:      false},
		{testName: "HTTPS issuer with wildcard trusted participant URL should be trusted.",
			credentialToVerifiy: getCredential("https://any-issuer.example.com"),
			verificationContext: TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "ebsi", Url: "*"}}}},
			expectedResult:      true},
		{testName: "HTTPS issuer should be validated by URL matching regardless of trust list type.",
			credentialToVerifiy: getCredential("https://issuer.example.com"),
			verificationContext: TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "gaia-x", Url: "https://issuer.example.com"}}}},
			expectedResult:      true},
		{testName: "HTTPS issuer matching one of multiple trusted participant URLs should be trusted.",
			credentialToVerifiy: getCredential("https://issuer-b.example.com"),
			verificationContext: TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "ebsi", Url: "https://issuer-a.example.com"}, {Type: "ebsi", Url: "https://issuer-b.example.com"}}}},
			expectedResult:      true},
		{testName: "DID issuer should still use type-based dispatch when HTTPS entries are also present.",
			credentialToVerifiy:  getCredential("did:web:trusted-issuer.org"),
			verificationContext:  TrustRegistriesValidationContext{trustedParticipantsRegistries: map[string][]config.TrustedParticipantsList{"someType": {{Type: "ebsi", Url: "https://issuer.example.com"}, {Type: "ebsi", Url: "http://my-trust-registry.org"}}}},
			ebsiParticipantsList: []string{"did:web:trusted-issuer.org"},
			expectedResult:       true},
	}
