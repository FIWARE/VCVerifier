...
func (v *CredentialVerifier) GenerateToken(clientId, subject, audience string, scopes []string, verifiablePresentation *verifiable.Presentation) (int64, string, error) {
	...
	for _, vc := range verifiablePresentation.Credentials() {
		...
		// gaia-x credentials have there type in the subjec at the type-field
		for _, subject := range vc.Contents().Subject {
			subjectType := subject.CustomFields["type"].(string)
			if _, ok := credentialsByType[subjectType]; !ok {
				credentialsByType[subjectType] = []*verifiable.Credential{}
			}
			credentialsByType[subjectType] = append(credentialsByType[subjectType], vc)
			credentialTypes = append(credentialTypes, subjectType)
		}
		credentialTypes = append(credentialTypes, vc.Contents().Types...)
	}
	logging.Log().Debugf("The types %s", credentialTypes)

	credentialsToBeIncluded := []interface{}{}
	...
	for _, scope := range scopes {
		...
		for _, credential := range credentialsNeededForScope {
			typesOfCredential := v.getTypesForCredential(credential)
			holderValidationContexts, err := v.getHolderValidationContext(clientId, scope, typesOfCredential, holder)
			if err != nil {
			...
			shouldBeIncluded, inclusionConfig := v.shouldBeIncluded(clientId, scope, typesOfCredential)
			if shouldBeIncluded {
	...
	token, err := v.generateJWT(credentialsToBeIncluded, subject, audience)
	if err != nil {
...
func buildInclusion(credential *verifiable.Credential, inclusionConfig config.JwtInclusion) (inclusion map[string]interface{}) {
	...
	for _, fullPath := range inclusionConfig.ClaimsToInclude {
		pathParts := strings.Split(fullPath, ".")
		if val, ok := getValueFromPath(credential.ToRawJSON(), pathParts); ok {
			setValueAtPath(inclusion, pathParts, val)
		}
	}
	return inclusion
}
...
func setValueAtPath(m map[string]interface{}, path []string, value interface{}) {
	...
	for i, key := range path {
		...
		next, _ := curr[key].(map[string]interface{})
		curr = next
	}
}

func (v *CredentialVerifier) getTypesForCredential(credential *verifiable.Credential) []string {
	credentialTypes := []string{}
	credentialTypes = append(credentialTypes, credential.Contents().Types...)
	// gaia-x credentials have there type in the subjec at the type-field
	for _, subject := range credential.Contents().Subject {
		subjectType := subject.CustomFields["type"].(string)
		credentialTypes = append(credentialTypes, subjectType)
	}
...
func (v *CredentialVerifier) getTrustRegistriesValidationContextFromScope(clientId string, scope string, credentialTypes []string) (verificationContext TrustRegistriesValidationContext, err error) {
	...
	logging.Log().Debugf("Contained types %s", credentialTypes)
...
func (v *CredentialVerifier) generateJWT(credentials []interface{}, holder string, audience string) (generatedJwt jwt.Token, err error) {
