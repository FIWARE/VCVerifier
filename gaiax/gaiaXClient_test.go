...
func (r *mockRegistry) Resolve(didStr string) (*did.DocResolution, error) {
	if err, ok := r.errors[didStr]; ok && err != nil {
		return nil, err
	}
	return r.didDocs[didStr], nil
}
	...
	for _, tc := range tests {
		t.Run(tc.testName, func(t *testing.T) {
			gaiaXClient := GaiaXHttpClient{
				...
				didRegistry: &mockRegistry{didDocs: tc.didDocs, errors: tc.didErrors},
			}
...
func getDidDoc(controllerDID string) *did.DocResolution {
	// Build a JWK with x5u set
	...
	vm, err := did.NewVerificationMethodFromJWK(controllerDID, "JsonWebKey2020", controllerDID, key)
	if err != nil {
		return nil
	}
...
func getDidDocWithoutX5U(controllerDID string) *did.DocResolution {
	// Build a JWK without x5u
